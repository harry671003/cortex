	...
	tests := map[string]struct {
		...
		"should return series on success": {
			series: &typespb.Series{
				...
				Chunks: []typespb.AggrChunk{
					{MinTime: minTimestamp.Unix() * 1000, MaxTime: maxTimestamp.Unix() * 1000, Raw: &typespb.Chunk{Type: typespb.XOR, Data: mockTSDBChunkData()}},
				},
			},
			expectedMetric: labels.Labels{
				{Name: "foo", Value: "bar"},
		...
		"should return error on failure while reading encoded chunk data": {
			series: &typespb.Series{
				...
				Chunks: []typespb.AggrChunk{
					{MinTime: minTimestamp.Unix() * 1000, MaxTime: maxTimestamp.Unix() * 1000, Raw: &typespb.Chunk{Type: typespb.XOR, Data: []byte{0, 1}}},
				},
			},
...
func createAggrChunkWithSineSamples(minTime, maxTime time.Time, step time.Duration) typespb.AggrChunk {
...
func createAggrChunkWithSamples(samples ...promql.FPoint) typespb.AggrChunk {
	return createAggrChunk(samples[0].T, samples[len(samples)-1].T, samples...)
}

func createAggrChunk(minTime, maxTime int64, samples ...promql.FPoint) typespb.AggrChunk {
...
func Benchmark_blockQuerierSeriesSet_iteration(b *testing.B) {
	...
	// Generate series.
	series := make([]*typespb.Series, 0, numSeries)
	for seriesID := 0; seriesID < numSeries; seriesID++ {
		...
		series = append(series, &typespb.Series{
			Labels: lbls,
			Chunks: chunks,
		})
	}
